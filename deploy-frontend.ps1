...
aws s3 cp "$SrcDir\index.html"    "s3://$BucketName/index.html"    --content-type "text/html"             --region $Region --profile $Profile | Out-Null
aws s3 cp "$SrcDir\manifest.json" "s3://$BucketName/manifest.json" --content-type "application/json"       --region $Region --profile $Profile 2>$null | Out-Null
aws s3 cp "$SrcDir\sw.js"         "s3://$BucketName/sw.js"         --content-type "application/javascript" --region $Region --profile $Profile 2>$null | Out-Null
